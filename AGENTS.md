...
- `Dockerfile` builds a FrankenPHP image (`dunglas/frankenphp:1-php8.5-bookworm`, vite assets and composer vendor in earlier stages); `etc/entrypoint.sh` migrates and caches config/routes/views at boot; the health endpoint is `/ping.php`.
